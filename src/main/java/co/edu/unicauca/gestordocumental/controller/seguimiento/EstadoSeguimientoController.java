...
@Controller
@RequestMapping(path="/estado_seguimiento")
...
public class EstadoSeguimientoController {
    ...
    public @ResponseBody String listar() {
        ...
        List<EstadoSeguimiento> estados = this.estadoSeguimientoRepo.findAll();
        if (estados.size() > 0) {

            rta.put("estado", "exito");
            rta.put("data", estados);
            rta.put("mensaje", "Lista de estados seguimiento");

        } else {
            rta.put("estado", "fallo");
            rta.put("data", "");
            rta.put("mensaje", "No existen estados seguimiento");
        }
